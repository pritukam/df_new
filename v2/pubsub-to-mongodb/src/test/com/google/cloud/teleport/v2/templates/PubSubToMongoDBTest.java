...
public class PubSubToMongoDBTest {
    ...
    public void testPubSubToMongoDBNoUdfE2E() {
        ...
        coderRegistry.registerCoderForType(
                PubSubToMongoDB.FAILSAFE_ELEMENT_CODER.getEncodedTypeDescriptor(), PubSubToMongoDB.FAILSAFE_ELEMENT_CODER);
        ...
        PubSubToMongoDB.Options options =
                TestPipeline.testingPipelineOptions()
                        .as(PubSubToMongoDB.Options.class);
    ...
    /** Tests the {@link PubSubToMongoDB} pipeline end-to-end with an empty message payload but attributes populated. */
    @Test
    ...
    /**
     * Tests the {@link PubSubToMongoDB} pipeline end-to-end for payload to mongoDB document conversion.
     */
    @Test
    public void testPubSubToMongoDBDocumentParser() {
        CoderRegistry coderRegistry = pipeline.getCoderRegistry();
        coderRegistry.registerCoderForType(
                PubSubToMongoDB.FAILSAFE_ELEMENT_CODER.getEncodedTypeDescriptor(), PubSubToMongoDB.FAILSAFE_ELEMENT_CODER);
        coderRegistry.registerCoderForType(
        ...
        PAssert.that(pc.get(PubSubToMongoDB.TRANSFORM_OUT)
                .apply(MapElements.into(TypeDescriptors.strings()).via(FailsafeElement::getPayload))
                .apply(ParDo.of(new PubSubToMongoDB.ParseAsDocumentsFn()))).satisfies(collection->{
                    Document element = collection.iterator().next();
                    assertThat(element.toJson().replace(" ", ""), is(equalTo((new String (goodTestMessages.get(0).getPayload()).replace(" ", "")))));
                    return null;
        });
